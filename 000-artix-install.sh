...
EFI_PART=$(lsblk -o NAME,LABEL,FSTYPE,SIZE -nr | grep -i "efi" | awk '$3=="vfat" && $1 !~ /^sdx/ {print "/dev/" $1}')
if [ -z "$EFI_PART" ]; then
...
ROOT_PART=$(lsblk -o NAME,LABEL,FSTYPE,SIZE -nr | grep -i "ROOT" | awk '$3=="ext4" && $1 !~ /^sdx/ {print "/dev/" $1}')
if [ -z "$ROOT_PART" ]; then
...
HOME_PART=$(lsblk -o NAME,LABEL,FSTYPE,SIZE -nr | grep -i "home" | awk '$3=="ext4" && $1 !~ /^sdx/ {print "/dev/" $1}')
if [ -z "$HOME_PART" ]; then
...
if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
    basestrap $MOUNT_POINT base base-devel runit elogind-runit linux linux-firmware nano intel-ucode git
